...
#define CHANGE_NAME(y) Zombie ## _ ## y
...
void CHANGE_NAME (Render)(sEnemy * self, int position)
{
    ...
    {
        take_damage(self->lane, ELEMENT_NONE, 4);
        next_turn();
    }

    pos.x -= move_forward_animation(self->attack_animation_timer, 80);

    drawTextureEx(_goblin1_sprite, pos, 0, 0.1, GREEN);
...
char CHANGE_NAME(description)[] = "Zombie\n10 health\nDeals 20 dmg\nWeak to Fire";

sEnemy _prefab_zombie = {
    .active = true,
    .health = 5,
    .lane = 0,
